...
public class SineGenerator : SynthProvider
{
    ...
    private delegate double BurstSineDelegate(ref SynthBuffer buffer, double phase, int sampleRate, float amplitude, float frequency);

    [BurstCompile]
    private static double BurstSine(ref SynthBuffer buffer, double phase, int sampleRate, float amplitude, float frequency)
    {
        ...
        for (int sample = 0; sample < buffer.Length; sample += buffer.Channels)
        {
            ...
            float value = (float)(math.sin(phase * 2 * math.PI));
    ...
    {
        _phase = BurstSine(ref buffer, _phase, _sampleRate, amplitude, frequency);
    }
}
